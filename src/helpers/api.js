...
class API {
    ...
    static loadCategoriesFromServer() {
        return fetch(`${API.serverHost}/categories`, {headers: {'Authorization': 'whatever-you-want'}})
            .then((response) => {
                if (!response.ok) {
    ...
    static loadPostsFromServer() {
        return fetch(`${API.serverHost}/posts`, {headers: {'Authorization': 'whatever-you-want'}})
            .then((response) => {
                if (!response.ok) {
                    throw Error(response.statusText);
                }
    ...
    static requestDeletePost(postId){
        return fetch(`${API.serverHost}/posts/${postId}`, {method: 'DELETE', headers: {'Authorization': 'whatever-you-want'}})
            .then((response) => {
                if (!response.ok) {
                    throw Error(response.statusText);
                }
